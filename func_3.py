...
def func_3(graph, node, dest):
    ...
    if nx.has_path(graph,node,dest):               #if they do and they are connected
        ...
        # We need this part to get the path to reach dest (we go back from  
        # dest to start through the previous list and then reverse the list)
        path=[]
        index=dest
        node=previous[index]
        while index!=node:
    ...
    #if the nodes are not connected it's impossible to find the path
    else:                     
        ret=('Not possible')
